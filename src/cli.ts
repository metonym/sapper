...
import { rimraf } from "./api/utils/fs_utils";
...
prog
  ...
  .option(
    "--output",
    "Sapper intermediate file output directory",
      ...
      try {
        ...
        watcher.on("ready", async (event: ReadyEvent) => {
          if (first) {
            console.log(
              colors
                .bold()
                .cyan(`> Listening on http://localhost:${event.port}`)
            );
            if (opts.open) {
        ...
        watcher.on("error", (event: ErrorEvent) => {
          ...
          if (error.loc && error.loc.file) {
            console.log(
              colors.bold(
                `${path.relative(process.cwd(), error.loc.file)} (${
                  error.loc.line
                }:${error.loc.column})`
              )
            );
        ...
        watcher.on("build", (event: BuildEvent) => {
          if (event.errors.length) {
            ...
            if (hidden > 0) {
              console.log(
                `${hidden} duplicate ${
                  hidden === 1 ? "warning" : "warnings"
                } hidden\n`
              );
            }
          } else {
            console.log(
              `${colors.bold().green(`✔ ${event.type}`)} ${colors.gray(
                `(${format_milliseconds(event.duration)})`
              )}`
            );
          }
        });
...
prog
  ...
  .option(
    "--ext",
    "Custom page route extensions (space separated)",
    ".svelte .html"
  )
  ...
  .action(
    async (
      ...
      try {
        ...
        console.error(
          `\n> Finished in ${elapsed(start)}. Type ${colors
            .bold()
            .cyan(`node ${dest}`)} to run the app.`
        );
      } catch (err) {
        console.log(`${colors.bold().red(`> ${err.message}`)}`);
        console.log(colors.gray(err.stack));
...
prog
  ...
  .option(
    "--timeout",
    "Milliseconds to wait for a page (--no-timeout to disable)",
    5000
  )
  .option("--legacy", "Create separate legacy build")
  .option("--bundler", "Specify a bundler (rollup or webpack, blank for auto)")
  ...
  .action(
    async (
      ...
      try {
          ...
          onfile: (event) => {
            ...
            const file_label =
              event.status === 200
                ...
                : colors
                    .bold()
                    [event.status >= 400 ? "red" : "yellow"](
                      `(${event.status}) ${event.file}`
                    );
        ...
        const sapper_dir = path.resolve(opts.cwd, "__sapper__");
        ...
        console.error(
          `\n> Finished in ${elapsed(start)}. Type ${colors
            .bold()
            .cyan(`npx serve ${dest}`)} to run the app.`
        );
      } catch (err) {
...
prog.parse(process.argv, {
  unknown: (arg: string) => `Unknown option: ${arg}`,
});
