...
class Watcher extends EventEmitter {
  ...
  constructor({
    ...
    if (template.indexOf("%sapper.base%") === -1) {
      const error = new Error(
        `As of Sapper v0.10, your template.html file must include %sapper.base% in the <head>`
      );
  ...
  async init() {
    ...
    const compilers: Compilers = await create_compilers(
      this.bundler,
      cwd,
      src,
      dest,
    ...
    this.watch(compilers.client, {
      ...
      handle_result: (result: CompileResult) => {
        ...
        const client_files = result.chunks.map(
          (chunk) => `client/${chunk.file}`
        );
