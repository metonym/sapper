...
async function _export({
  ...
  const proc = child_process.fork(
    path.resolve(`${build_dir}/server/server.js`),
    [],
    {
      cwd,
      env: Object.assign(
  ...
  async function handle(url: URL) {
      ...
      if (type === "text/html") {
        ...
        link.refs.forEach((ref: Ref) => {
          if (ref.rel === "preload") {
            body = body.replace(
              "</head>",
              `<link rel="preload" as=${JSON.stringify(
                ref.as
              )} href=${JSON.stringify(ref.uri)}></head>`
            );
          }
        });
    ...
    if (range === 3) {
      ...
      type = "text/html";
      body = `<script>window.location.href = "${location.replace(
        origin,
        ""
      )}"</script>`;
