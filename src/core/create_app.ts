...
import { PageComponent, ManifestData } from "../interfaces";
...
export function create_app({
  ...
  const client_manifest = generate_client_manifest(
    manifest_data,
    path_to_routes,
    bundler,
    dev,
  ...
  const server_manifest = generate_server_manifest(
    manifest_data,
    path_to_routes,
    cwd,
    src,
...
function generate_client_manifest(
  ...
  const server_routes_to_ignore = manifest_data.server_routes.filter(
  ...
  return `
		...
		export { default as Root } from '${stringify(
      get_file(path_to_routes, manifest_data.root),
      false
    )}';
		export { preload as root_preload } from '${
      manifest_data.root.has_preload
        ? stringify(get_file(path_to_routes, manifest_data.root), false)
        : "./shared"
    }';
		export { default as ErrorComponent } from '${stringify(
      get_file(path_to_routes, manifest_data.error),
      false
    )}';

		export const ignore = [${server_routes_to_ignore
      ...
      dev
        ? `if (typeof window !== 'undefined') {
			import(${stringify(
        posixify(path.resolve(__dirname, "../sapper-dev-client.js"))
      )}).then(client => {
...
function generate_server_manifest(
  ...
  const imports = [].concat(
    manifest_data.server_routes.map(
      (route, i) =>
        `import * as route_${i} from ${stringify(
          posixify(`${path_to_routes}/${route.file}`)
        )};`
    ),
    manifest_data.components.map(
      (component, i) =>
        `import component_${i}${
          component.has_preload ? `, { preload as preload_${i} }` : ""
        } from ${stringify(get_file(path_to_routes, component))};`
    ),
    `import root${
      manifest_data.root.has_preload ? `, { preload as root_preload }` : ""
    } from ${stringify(get_file(path_to_routes, manifest_data.root))};`,
    `import error from ${stringify(
      get_file(path_to_routes, manifest_data.error)
    )};`
  );
              ...
              .map((part) => {
                ...
                const props = [
                  ...
                  `component: component_${
                    component_lookup[part.component.name]
                  }`,
                  part.component.has_preload &&
                    `preload: preload_${component_lookup[part.component.name]}`,
                ].filter(Boolean);
